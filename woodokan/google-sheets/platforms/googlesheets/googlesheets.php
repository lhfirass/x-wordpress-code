...
class woodokan_GoogleSheets extends woodokan_google_sheets_integration_OAuth2
{
    ...
    private function __construct()
    {
        ...
        $option = (array) maybe_unserialize( get_option( 'woodokan_googlesheets_keys' ) );
        ...
        add_filter(
            'woodokan_action_providers',
            array( $this, 'woodokan_googlesheets_actions' ),
            10,
            1
        );
        add_filter(
            'woodokan_settings_tabs',
            array( $this, 'woodokan_googlesheets_settings_tab' ),
            10,
            1
        );
        add_action(
            'woodokan_settings_view',
            array( $this, 'woodokan_googlesheets_settings_view' ),
            10,
            1
        );
        add_action(
            'admin_post_woodokan_save_googlesheets_keys',
            array( $this, 'woodokan_save_googlesheets_keys' ),
            10,
            0
        );
        add_action(
            'woodokan_action_fields',
            array( $this, 'action_fields' ),
            10,
            1
        );
        add_action(
            'wp_ajax_woodokan_get_spreadsheet_list',
            array( $this, 'get_spreadsheet_list' ),
            10,
            0
        );
        add_action(
            'wp_ajax_woodokan_googlesheets_get_worksheets',
            array( $this, 'get_worksheets' ),
            10,
            0
        );
        add_action(
            'wp_ajax_woodokan_googlesheets_get_headers',
    ...
    {
        register_rest_route( 'woodokanformintegration', '/googlesheets', array(
            'methods'             => 'GET',
    ...
    public function get_webhook_data( $request )
    {
        ...
        if ( $code ) {
            $redirect_to = add_query_arg( [
                'service' => 'authorize',
                'action'  => 'woodokan_googlesheets_auth_redirect',
                'code'    => $code,
    ...
    public function auth_redirect()
    {
        ...
        if ( 'woodokan_googlesheets_auth_redirect' == $action ) {
            $code = ( isset( $_GET['code'] ) ? sanitize_text_field( $_GET['code'] ) : '' );
            if ( $code ) {
                $this->request_token( $code );
            }
            wp_safe_redirect( admin_url( 'admin.php?page=woodokan-sheets-integration&tab=googlesheets' ) );
            exit;
        }
    
    }
    
    public function woodokan_googlesheets_actions( $actions )
    {
    ...
    public function woodokan_googlesheets_settings_tab( $providers )
    {
        $providers['googlesheets'] = __( 'Google Sheets', 'advanced-form-integration' );
        return $providers;
    }
    
    public function woodokan_googlesheets_settings_view( $current_tab )
    {
        if ( $current_tab != 'googlesheets' ) {
            return;
        }
        $option = (array) maybe_unserialize( get_option( 'woodokan_googlesheets_keys' ) );
        $nonce = wp_create_nonce( "woodokan_googlesheets_settings" );
        ...
        ?>"
            ...
            <input type="hidden" name="action" value="woodokan_save_googlesheets_keys">
        ...
        ?>
                                      <?php 
        _e( 'You can also check this video instruction <a target="_blank" style="    display: inline-block;
        padding: 5px;
        background-image: linear-gradient(178deg, red, #8a0505);
        color: white;
        text-decoration: none;
        border-radius: 5px;"rel="noopener noreferrer" href="https://youtu.be/VyLVTjQw0ks" > <span class="dashicons dashicons-video-alt3"></span> Whatch tutorial on youtube</a>', 'advanced-form-integration' );
        ?>
                        </p>
                    </td>
                </tr>
        ...
        ?></th>
                    <td>
                        <input type="text" name="woodokan_googlesheets_client_id"
                               value="<?php 
        ...
        ?></th>
                    <td>
                        <input type="text" name="woodokan_googlesheets_client_secret"
                               value="<?php 
    ...
    public function woodokan_save_googlesheets_keys()
    {
        // Security Check
        if ( !wp_verify_nonce( $_POST['_nonce'], 'woodokan_googlesheets_settings' ) ) {
            die( __( 'Security check Failed', 'advanced-form-integration' ) );
        }
        $client_id = ( isset( $_POST["woodokan_googlesheets_client_id"] ) ? sanitize_text_field( $_POST["woodokan_googlesheets_client_id"] ) : "" );
        $client_secret = ( isset( $_POST["woodokan_googlesheets_client_secret"] ) ? sanitize_text_field( $_POST["woodokan_googlesheets_client_secret"] ) : "" );
        ...
        //        advanced_form_integration_redirect( "admin.php?page=woodokan-sheets-integration&tab=googlesheets" );
    }
    ...
    {
        $data = (array) maybe_unserialize( get_option( 'woodokan_googlesheets_keys' ) );
        ...
        update_option( 'woodokan_googlesheets_keys', maybe_serialize( $option ) );
    }
    ...
    {
        return site_url( '/wp-json/woodokanformintegration/googlesheets' );
    }
...
$googlesheets = woodokan_GoogleSheets::get_instance();
/*
 * Saves connection mapping
 */
function woodokan_googlesheets_save_integration()
{
    $params = array();
    parse_str( woodokan_sanitize_text_or_array_field( $_POST['formData'] ), $params );
    $trigger_data = ( isset( $_POST["triggerData"] ) ? woodokan_sanitize_text_or_array_field( $_POST["triggerData"] ) : array() );
    $action_data = ( isset( $_POST["actionData"] ) ? woodokan_sanitize_text_or_array_field( $_POST["actionData"] ) : array() );
    $field_data = ( isset( $_POST["fieldData"] ) ? woodokan_sanitize_text_or_array_field( $_POST["fieldData"] ) : array() );
    ...
    global  $wpdb ;
    $integration_table = $wpdb->prefix . 'woodokan_integration';
 ...
 */
function woodokan_googlesheets_send_data( $record, $submitted_data )
{
    $record_data = json_decode( $record["data"], true );
    if ( array_key_exists( "cl", $record_data["action_data"] ) ) {
        if ( $record_data["action_data"]["cl"]["active"] == "yes" ) {
            if ( !woodokan_match_conditional_logic( $record_data["action_data"]["cl"], $submitted_data ) ) {
                return;
            }
        }
    }
    ...
    if ( $task == "add_row" ) {
        ...
        if ( empty($data) ) {
            ...
            foreach ( $data as $key => $value ) {
                $holder[$key] = woodokan_get_parsed_values( $data[$key], $posted_data );
            }
        }
