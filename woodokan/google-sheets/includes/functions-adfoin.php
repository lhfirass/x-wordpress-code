...
*/
function woodokan_get_form_providers()
{
    $providers = array(
    ...
    // Retrieve the settings for supported action platforms.
    // $platform_settings = woodokan_get_action_platform_settings();
    // Check if WooCommerce is active and add it to the list of providers.
    ...
    // Merge the provider arrays and apply a filter.
    $providers = $providers + $pro_items;
    return apply_filters( 'woodokan_form_providers', $providers );
}
...
*/
function woodokan_get_form_providers_html()
{
    $form_providers = woodokan_get_form_providers();
...
*/
function woodokan_get_actions()
{
    $actions = array();
    return apply_filters( 'woodokan_action_providers', $actions );
}
...
*/
function woodokan_get_action_porviders()
{
    $actions = woodokan_get_actions();
    $providers = array();
...
*/
function woodokan_get_action_tasks( $provider = '' )
{
    $actions = woodokan_get_actions();
    $tasks = array();
    if ( $provider ) {
...
*/
function woodokan_get_settings_tabs()
{
    $tabs = array(
        'general' => __( 'General', 'advanced-form-integration' ),
    );
    return apply_filters( 'woodokan_settings_tabs', $tabs );
}
...
*/
function woodokan_get_action_platform_list()
{
    return array(
...
*/
function woodokan_get_action_platform_settings()
{
    global  $wpdb ;
    $settings = ( get_option( 'woodokan_general_settings_platforms' ) ? get_option( 'woodokan_general_settings_platforms' ) : array() );
    $saved_records = $wpdb->get_results( "SELECT form_provider, action_provider FROM {$wpdb->prefix}woodokan_integration WHERE status = 1", ARRAY_A );
...
*/
add_action(
    'woodokan_settings_view',
    'woodokan_general_settings_view',
    10,
    1
);
add_action(
    'woodokan_settings_view',
    'woodokan_integration_views',
    10,
    1
);
add_action(
    'woodokan_settings_view',
    'woodokan_integrationlog_views',
    10,
    1
);

function woodokan_integrationlog_views($current_tab){
    if ( $current_tab == 'integrationlog' ) {
    ?>
 <div class="wrap">
            ...
            <form id="form-list" method="post">
                ...
                <?php
                $list_table = new woodokan_google_sheets_integration_Log_Table();
...
function woodokan_integration_views( $current_tab){
 ...
 <div class="wrap">
            ...
            <form id="form-list" method="post">
                ...
                <?php
                $list_table = new woodokan_google_sheets_integration_List_Table();
                $list_table->prepare_items();
                $list_table->display();
                ?>
            </form>
...
* @return void
*/
function woodokan_general_settings_view( $current_tab )
{
    if ( $current_tab != 'general' ) {
        return;
    }
    $nonce = wp_create_nonce( "woodokan_general_settings" );
    $log_settings = ( get_option( 'woodokan_general_settings_log' ) ? get_option( 'woodokan_general_settings_log' ) : '' );
    $st_settings = ( get_option( 'woodokan_general_settings_st' ) ? get_option( 'woodokan_general_settings_st' ) : '' );
    $utm_settings = ( get_option( 'woodokan_general_settings_utm' ) ? get_option( 'woodokan_general_settings_utm' ) : '' );
    $platform_settings = woodokan_get_action_platform_settings();
    $platforms = woodokan_get_action_platform_list();
    ?>
    ...
    ?>"
        ...
        <input type="hidden" name="action" value="woodokan_save_general_settings">
    ...
    ?></h3>
            <div class="afi-checkbox-container">
                <div class="afi-checkbox">
                    <div class="afi-elements-info">
                        <p class="afi-el-title">
                            <label for="woodokan_disable_log"><?php 
    _e( 'Disable Log', 'advanced-form-integration' );
    ?></label>
                        </p>
                    </div>
                    <label class="adfoin-toggle-form form-enabled">
                    <input type="checkbox" value="1" id="woodokan_disable_log" name="woodokan_disable_log" <?php 
    ...
    ?>>
                ...
                <div class="afi-checkbox">
                    <div class="afi-elements-info">
                        <p class="afi-el-title">
                            <label for="woodokan_disable_st"><?php 
    _e( 'Disable Special Tags', 'advanced-form-integration' );
    ?></label>
                        </p>
                    </div>
                    <label class="adfoin-toggle-form form-enabled">
                    <input type="checkbox" value="1" id="woodokan_disable_st" name="woodokan_disable_st" <?php 
    ...
    ?>>
                ...
                <div class="afi-checkbox">
                    <div class="afi-elements-info">
                        <p class="afi-el-title">
                            <label for="woodokan_enable_utm"><?php 
    _e( 'Send UTM Variables', 'advanced-form-integration' );
    ?></label>
                        </p>
                    </div>
                    <label class="adfoin-toggle-form form-enabled">
                    <input type="checkbox" value="1" id="woodokan_enable_utm" name="woodokan_enable_utm" <?php 
...
add_action(
    'admin_post_woodokan_save_general_settings',
    'woodokan_save_general_settings',
    10,
    0
);
function woodokan_save_general_settings()
{
    // Security Check
    if ( !wp_verify_nonce( $_POST['_nonce'], 'woodokan_general_settings' ) ) {
        die( __( 'Security check Failed', 'advanced-form-integration' ) );
    }
    $log_settings = ( isset( $_POST["woodokan_disable_log"] ) ? sanitize_text_field( $_POST["woodokan_disable_log"] ) : '' );
    $st_settings = ( isset( $_POST["woodokan_disable_st"] ) ? sanitize_text_field( $_POST["woodokan_disable_st"] ) : '' );
    $utm_settings = ( isset( $_POST["woodokan_enable_utm"] ) ? sanitize_text_field( $_POST["woodokan_enable_utm"] ) : '' );
    $default_platforms = array_fill_keys( array_keys( woodokan_get_action_platform_list() ), false );
    $activated_platforms = ( isset( $_POST["platforms"] ) ? woodokan_sanitize_text_or_array_field( $_POST["platforms"] ) : array() );
    ...
    // Save
    // var_dump($all_platforms);exit;
    update_option( "woodokan_general_settings_platforms", array('googlesheets'=>true) );
    update_option( "woodokan_general_settings_log", $log_settings );
    update_option( "woodokan_general_settings_st", $st_settings );
    update_option( "woodokan_general_settings_utm", $utm_settings );
    advanced_form_integration_redirect( "admin.php?page=woodokan-sheets-integration" );
}
 ...
 */
function woodokan_sanitize_text_or_array_field( $array_or_string )
{
    ...
    if ( is_string( $array_or_string ) ) {
        ...
        foreach ( $array_or_string as $key => &$value ) {
            ...
            if ( is_array( $value ) ) {
                $value = woodokan_sanitize_text_or_array_field( $value );
            } else {
 ...
 * Get parsed value
 */
function woodokan_get_parsed_values( $field, $posted_data )
{
...
)
{
    ...
    $log = new woodokan_google_sheets_integration_Log();
...
function woodokan_get_cl_conditions()
{
    return array(
...
function woodokan_match_conditional_logic( $cl, $posted_data )
{
    if ( $cl["active"] != "yes" ) {
        return true;
    }
    ...
    foreach ( $cl["conditions"] as $condition ) {
        ...
        $field_value = woodokan_get_parsed_values( $field, $posted_data );
        if ( woodokan_match_single_logic( $field_value, $condition["operator"], $condition["value"] ) ) {
            $match++;
        }
    }
...
function woodokan_match_single_logic( $data, $operator, $value )
{
    $result = false;
    switch ( $operator ) {
        case 'equal_to':
...
function woodokan_get_special_tags( $cat = '' )
{
    $utm_tags = array();
    $special_tags = array();
    ...
    if ( '1' != get_option( 'woodokan_general_settings_st' ) ) {
...
function woodokan_get_special_tags_values( $post )
{
    $st_data = array();
    $utm_data = array();
    ...
    if ( '1' != get_option( 'woodokan_general_settings_st' ) ) {
        ...
        $special_tags = woodokan_get_special_tags( 'st' );
        if ( !empty($special_tags) ) {
            foreach ( $special_tags as $key => $value ) {
                $st_data[$key] = woodokan_get_single_special_tag_value( $key, $current_user, $post );
            }
        }
    }
...
function woodokan_get_single_special_tag_value( $tag, $current_user, $post )
{
    switch ( $tag ) {
...
function woodokan_is_valid_md5( $md5 = '' )
{
    return preg_match( '/^[a-f0-9]{32}$/', $md5 );
}

// Get saved UTM params
function woodokan_capture_utm_and_url_values()
{
    $fields = woodokan_get_special_tags( 'utm' );
    $cookie_fields = array();
...
*/
function woodokan_remote_request( $url, $args )
{
