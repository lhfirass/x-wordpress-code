 ...
 *
 */
class woodokan_google_sheets_integration_Admin_Menu {
    /**
     * Class constructor.
     */
    ...
    public function admin_menu() {
        ...
        remove_menu_page('advanced-form-integration');
    }

    public function enqueue_assets() {
        wp_enqueue_style( 'adfoin-main-style' );
        wp_enqueue_script( 'adfoin-vuejs' );
        do_action( 'woodokan_custom_script' );
     ...
     * @return void
     */
    public function woodokan_routing() {
        $action = isset( $_GET['action'] ) ? $_GET['action'] : 'list';
        $id     = isset( $_GET['id'] ) ? intval( $_GET['id'] ) : 0;

        switch ( $action ) {
            case 'edit':
                $this->woodokan_edit( $id );
                break;
            case 'duplicate':
                $this->woodokan_duplicate_integration($id);
                break;
            default:
                $this->woodokan_list_page() ;
                break;
        }
    }
     ...
     */
    public function woodokan_list_page() {
     ...
     */
    public function woodokan_new_integration(){

        $form_providers   = woodokan_get_form_providers();
        $action_providers = woodokan_get_action_porviders();
     ...
     */
    public function woodokan_view( $id='' ) {
    }

    /*
     * Handles connection edit
     */
    public function woodokan_edit( $id='' ) {
     ...
     */
    public function woodokan_settings( $value = '' ) {
        $tabs = woodokan_get_settings_tabs();
     ...
     * Log Submenu View
     */
    public function woodokan_log( $value = '' ) {
        ...
        switch ( $action ) {
            case 'view':
                $this->woodokan_log_view( $id );
                break;
            default:
                $this->woodokan_log_list_page() ;
                break;
        }
     ...
     */
    public function woodokan_get_help( $value = '' ) {
    ...
    * This function generates the list of connections
    */
    public function woodokan_log_list_page() {
        if ( isset( $_GET['status'] ) ) {
            $status = $_GET['status'];
        }
     ...
     * Handles log view
     */
    public function woodokan_log_view( $id='' ) {
    ...
    /*
     * Relation Status Change woodokan_status
     */
    public function woodokan_duplicate_integration( $id = '' ) {

        global $wpdb;

        $table         = $wpdb->prefix . "woodokan_integration";
