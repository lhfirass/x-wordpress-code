...
class woodokan_google_sheets_integration_Submission {

    public function __construct() {
        add_action( 'wp_ajax_woodokan_get_forms', array( $this,'get_forms' ) );
        add_action( 'wp_ajax_woodokan_get_form_fields', array( $this,'get_form_fields' ) );
        add_action( 'wp_ajax_woodokan_get_tasks', array( $this,'get_tasks' ) );
        add_action( 'admin_post_woodokan_save_integration', array( $this,'save_integration' ) );
        add_action( 'admin_post_woodokan_resend_log_data', array( $this,'resend_log_data' ) );
        add_action('wp_ajax_woodokan_enable_integration', array( $this, 'woodokan_enable_integration' ) );
    }
    ...
    public function get_forms() {
        ...
        if( $form_provider ) {
            $forms = call_user_func( "woodokan_{$form_provider}_get_forms", $form_provider );
    ...
    public function get_form_fields() {
        ...
        if( $form_provider && $form_id ) {
            $fields = call_user_func( "woodokan_{$form_provider}_get_form_fields", $form_provider, $form_id );
    ...
    public function get_tasks() {
        ...
        if( $action_provider ) {
            $tasks = woodokan_get_action_tasks( $action_provider );
    ...
    public function save_integration() {
        ...
        $type              = isset( $_POST['type'] ) ? woodokan_sanitize_text_or_array_field( $_POST['type'] ) : '';
    ...
    public function resend_log_data() {
        ...
        $response       = woodokan_remote_request( $url, $args );

        woodokan_add_to_log( $response, $url, $args, array( 'id' => $integration_id ), $log_id );
        wp_safe_redirect( admin_url( 'admin.php?page=advanced-form-integration-log&action=view&id=' . $log_id ) );
    }

    public function woodokan_enable_integration() {
        // Security Check
