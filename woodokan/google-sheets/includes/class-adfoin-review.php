 ...
 * @since 1.35.0
 */
class woodokan_google_sheets_integration_Review {
    ...
    public function __construct() {
        ...
        add_action( 'wp_ajax_woodokan_review_dismiss', array( $this, 'review_dismiss' ) );
    ...
    public function review_request() {
        ...
        $review = get_option( 'woodokan_review' );
        $time   = time();
        $load   = false;
        ...
        if ( ! $review ) {
            ...
            update_option( 'woodokan_review', $review );
        } else {
        ...
        $oldest_active = $wpdb->get_row( "SELECT *, DATE_FORMAT(time, '%m/%d/%Y') FROM {$wpdb->prefix}woodokan_integration WHERE status = 1 AND DATE(time) < CURDATE() - INTERVAL 30 DAY ORDER BY ID DESC", ARRAY_A );
        ...
        <script type="text/javascript">
            jQuery( function ( $ ) {
                $( document ).on( 'click', '.adfoin-dismiss-review-notice, .adfoin-review-notice button', function ( event ) {
                    ...
                    $.post( ajaxurl, {
                        action: 'woodokan_review_dismiss'
                    } );
    ...
    public function review_dismiss() {
        ...
        $review              = get_option( 'woodokan_review', array() );
        $review['time']      = time();
...
new woodokan_google_sheets_integration_Review;
